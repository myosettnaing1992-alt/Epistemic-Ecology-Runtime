...
Every result in the paper is generated by scripts in experiments/ and
benchmarks/. Run them in order:
...
# 5. Topology ablation (BA vs ER vs MVE)
python experiments/run_topology_ablation.py --quick --out results/topology_ablation.json

# 6. Scalability sweep (n = 500 -> 10^4)
python experiments/run_scalability.py --sizes 500 1000 2500 5000 --out results/scalability.json

# 7. Hyperparameter sensitivity (epsilon, M)
python experiments/run_sensitivity.py --n 500 --out results/sensitivity.json
```

Or, via the convenience shell script:
...
run_wallclock.py --all results/table_9_1.json Table 9.1
run_residual_decay.py figures/residual_decay.pdf Figure 11.1
run_topology_ablation.py results/topology_ablation.json Section 7.3
run_scalability.py results/scalability.json Section 7.4
run_sensitivity.py results/sensitivity.json Section 7.5
...
Note on the 11x figure. The 11x speedup reported in the paper is
specific to BA scale-free graphs with n = 500 and residual tolerance
1e-6, measured in coordinate updates (15,500 / 1,403 = 11.0). On
larger graphs (n = 10^4) the update-count speedup is ~1.6x, and on
Erdos-Renyi graphs it vanishes. Wall-clock speedup is smaller than
update-count speedup because the priority queue introduces O(log n)
overhead per pop. See benchmarks/results/sample_benchmark.md for the
full breakdown.
...
Distributed under the MIT License. If you use EER in your research,
please cite:
